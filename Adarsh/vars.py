...
class Var(object):
    ...
    # Your VPS IP address
    VPS_IP = '194.87.18.248'
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', VPS_IP))
    ...
    # Use your VPS IP for FQDN when not on Heroku
    FQDN = str(getenv('FQDN', f'{VPS_IP}:{PORT}')) if not ON_HEROKU or getenv('FQDN') else APP_NAME+'.herokuapp.com'
